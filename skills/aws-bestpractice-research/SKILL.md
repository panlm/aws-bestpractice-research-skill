...
### Step 2: Parallel Documentation Search

Run **all 6 search queries in parallel** using `aws___search_documentation`. Replace `{SERVICE}` with
...
From the search results, identify and read the most important pages in parallel using
`aws___read_documentation`. Prioritize these document types:
...
- **Use parallel searches**: All 6 initial searches should run in parallel to save time.
  Page reads should also be parallelized where possible.
